...
def new_block_received():
    ...
    if not valid:
        #.warning("[RPC: inform/block] Invalid block")
    ...
    if block.number == 1:
        blockchain.state.balances["A"] = 10000
        blockchain.state.history_list["A"] = [[1, 10000]]
    else:
    ...
    blockchain.iteration+=1
    if blockchain.node_identifier == blockchain.nodes[blockchain.iteration % num_nodes]:
